...
- **Voice Command** : Press and hold the **[ ` ]**(BackQuote) key to activate your microphone and give voice commands to your AI companion. Release the key to stop listening.
...
1.  **STT (Speech-to-Text):**
    * Voice audio data is captured from the player's microphone.
    * This data is sent to the **OpenAI Whisper** model, which transcribes it into text data in real-time.

2.  **LLM (Large Language Model):**
    * The text transcribed by the STT is passed to the **Qwen3** LLM.
    * This model is merged with a **LoRA Adapter** that has been fine-tuned for in-game commands.
    * The LLM interprets the player's textual intent and outputs the result as **Response Data (JSON)**, which includes the AI's actions and spoken responses.

3.  **Action (FSM-LLM Interaction):**
    * The JSON data generated by the LLM is fed into a **Finite State Machine (FSM)**.
    * The FSM parses this data to determine and execute the specific actions (e.g., move, attack, defend) that the AI "Actor" will perform in the game.

4. **Caching & Pre-caching:**
    * To optimize voice generation performance, a **Pre-caching** system is used.
    * This system uses Spark-TTS to pre-generate core vocabulary and voice resources for each AI, storing them in **Cache Data**.
    * The TTS module utilizes this cache to generate speech more quickly and efficiently during gameplay.
    
5. **TTS (Text-to-Speech):**
    * The Action system sends the AI's dialogue text to the **Spark-TTS** module.
    * Spark-TTS generates the AI companion's voice based on this text, which is then played back to the player through their speakers.
