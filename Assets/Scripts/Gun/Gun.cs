...
public class Gun : MonoBehaviour
{
    ...
    {
        // 현재 상태가 발사 가능한 상태이고, 쿨다운(timeBetFire)이 지났는지 확인
        if (state == State.Ready && Time.time >= lastFireTime + timeBetFire)
        {
            // 1) 총구 정면(fireTransform.forward)을 기본으로 잡는다.
            Vector3 fireDirection = fireTransform.forward.normalized;
    ...
    private void DrawPreviewLine()
    {
        ...
        Vector3 targetPoint = start + dir * fireDistance;
